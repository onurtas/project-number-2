...
def draw_speedometer(ax, value, avg_30d, avg_6m, scope_label, n_articles, window_used):
    ...
    if value is None:
        vc, lt = "#9CA3AF", ar("لا توجد بيانات")
    elif val <= -3:
        vc, lt = "#DC2626", ar("هبوط")
    elif val <= -1:
        vc, lt = "#EA580C", ar("هبوط طفيف")
    elif val <= 1:
        vc, lt = "#D97706", ar("محايد")
    elif val <= 3:
        vc, lt = "#059669", ar("صعود طفيف")
    else:
        vc, lt = "#16A34A", ar("صعود")
    ...
    window_tr = ar("نافذة 6 ساعات") if window_used == "6h" else ar("نافذة 24 ساعة")
...
fig.suptitle(ar("مؤشر معنويات العملات الرقمية"),
...
draw_speedometer(ax2,
    ...
    scope_label=ar("الولايات المتحدة"),
...
png_path = OUTDIR / f"sentiment_gauge_ar_{tag}.png"
...
json_path = OUTDIR / f"sentiment_gauge_ar_{tag}.json"
...
def format_tweet(gauge_global, gauge_us, window_end):
    def tone_label(val):
        ...
        if val <= -3: return "هبوط"
        if val <= -1: return "هبوط طفيف"
        if val <= 1: return "محايد"
        if val <= 3: return "صعود طفيف"
        return "صعود"
    def arrow(c, b):
...
post_path = OUTDIR / f"sentiment_gauge_ar_{tag}_post.json"
