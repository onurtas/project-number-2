...
MAX_CACHE_AGE_HOURS = 26  # AR is read-only: beyond this, full-query fallback
...
query_mode = None
try:
    if cache is not None and cache_age_h < MAX_CACHE_AGE_HOURS:
        query_mode = "CACHED"
        cur_df = run_query(current_sql, "current window")
        df = merge_current_with_baseline(cur_df, cache)
    else:
        query_mode = "FALLBACK"
        reason = load_err if cache is None else f"stale ({cache_age_h:.1f}h >= {MAX_CACHE_AGE_HOURS}h)"
        print(f"Cache unusable ({reason}) -> combined query")
